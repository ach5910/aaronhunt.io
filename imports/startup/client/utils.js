...
function rippleColor(bg) {
    return function rippleCallback(cb, delay = 0, className = "") {
        return function rippleAnimate(e) {
            ...
            const removeNode = (e) => {
                ...
                const {target} = e;
                e.target.removeEventListener(getTransitionEvent(target), removeNode);
            ...
            ripple.setAttribute(
                "style",
                `background-color: ${bg}; top: ${offsetY}px; left: ${offsetX}px; transform: scale(10); webkitTransform: scale(10); msTransform: scale(10); opacity: 0;`
            );
